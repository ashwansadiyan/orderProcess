$(document).ready(function () {
    ...
    $("#btnInPrg").removeClass("btn-danger").addClass("btn-primary");
    $("#btnInQue").removeClass("btn-primary").addClass("btn-danger");
    $("#btnPrPd").removeClass("btn-primary").addClass("btn-danger");
    $("#btnCnceld").removeClass("btn btn-primary").addClass("btn-danger");
...
function updateOrder(e, g) {
    ...
    $("#processHeader").html("Prepared :" + hding);
    $("#orderID").val(g);
    $("#State").val(3);
...
function bindOrderProcceingDetails() {
    ...
    $.ajax({
        ...
        url: "http://localhost:32314/Api/Orders/GetCustomerOrderBystatus?Id=2",
        aync: false,
        success: function (t) {
            if (t.length > 0) {
                orderObject = t;
    ...
    for (var i = 0; i < orderObject.length; i++) {
        ...
        var tableBodyClose = "<tr style='background-color:#337ab7' class='text-light' >" +
            "<td colspan='3')>" +
            "<button type='button' onclick='updateOrder(this," + orderID + ");' style='width:100%;height:35px;' class='btn btn-primary'>Prepare</button>" +
